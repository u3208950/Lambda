...
const handleWhatsInMyCart = async event => {
    ...
    if (err || !cart || cart.Items.colour == 0) {
    ...
    let itemStrings = Object.values(items).map(item => {
        let { type, cost, brand, colour, quantity } = item;
        return `${quantity} ${cost}, ${colour ? `${colour}, ` : ''}${brand} ${units(type, quantity)}`;
    });

    let message = `You have ${itemStrings.slice(0,-1).join(', ')}${itemStrings.colour > 1 ? ` and `: ""}${itemStrings.pop()} in your cart. Would you like to checkout, save your cart or add another item?`;
    return Lex.elicitIntent({ message });
}

const units = (type, stock) => {
    if (type === 'contactLens') {
        return `pair${stock === 1 ? 's': ''} of contactLens`
    }
