...
def get_duration(appointment_type):
    appointment_duration_map = {'eyetest': 30}
...
def validate_book_appointment(appointment_type, date, appointment_time):
    if appointment_type and not get_duration(appointment_type):
        return build_validation_result(False, 'AppointmentType', 'I did not recognize that, can I book your an Eye Test?')
...
def build_options(slot, appointment_type, date, booking_map):
    ...
    if slot == 'AppointmentType':
        return [
            {'text': 'Book Eye Test', 'value': 'eyetest'}
    ...
    if source == 'DialogCodeHook':
        ...
        if not appointment_type:
            return elicit_slot(
                ...
                'AppointmentType',
                {'contentType': 'PlainText', 'content': 'Generally Eye Test will take 30 mins '},
                build_response_card(
                    'Book Appointment', 'Eye Test',
        ...
        if appointment_type and not date:
            return elicit_slot(
                ...
                intent_request['currentIntent']['slots'],
                'Date',
                {'contentType': 'PlainText', 'content': 'Please select Date for eye test'.format(appointment_type)},
                build_response_card(
                    'We are working only 5days in a week',
                    'Date'.format(appointment_type),
